...
(function (win) {
    ...
    win.wordle=function(wordList,option,textGroup,classname="word",exist=[]){ //exist参数是为了适应于画布上已有word的情况
        ...
        let words2DInfo=d3.range(wordList.length).map(function(d,i){
            ...
            if(wordList[d].color){
                // console.log(color)
               word2D["color"]=wordList[d].color;//(Math.random()*0xffffff<<0).toString(16); 
            }
            else{
                word2D["color"]=option.color;
            }
            word2D["fontStyle"]=option["font-family"];
            if(wordList[d].rotate){
                word2D["rotation"]=wordList[d].rotate;
            }
            else{
                word2D["rotation"]=option.rotate;
            }
            word2D['fit']=true;
            ...
            function isInBound(){
                ...
                for(let rect of word.leafArray){
                    ...
                    let rBound=boundary.func(theta).map(function (r) {
                        return r*boundary.maxR;//todo 留3px的间隙
                    })
                    // console.log(rBound)
