...
//生成页面框架
//todo 增加一个出画布检测 不能出画布  svg主画布改成可伸缩的 实际大小可以很大，显示在页面上的为一个固定大小的窗口?->注意这样易导致文字重叠
(function initialLayout() {
    vis = d3.select("#vis"); //TODO 应该声明为全局变量吗？

    vis.attr("width",WIDTH).attr("height",HEIGHT);
    vis.attr('viewBox','0 0 '+WIDTH+' '+HEIGHT);
    vis.attr("width",'990').attr("height",'660');
    vis.append('g').attr('class','bubbles');
...
function addAlabel(station,regions,origin) {
    ...
    station['wordDic']=getWordDict(station,"美食",2000);
    ...
    document.querySelector(".stations").addEventListener("click",function(e) {
        if(e.target && e.target.nodeName == "circle") {
            ...
            if(sWithLabel.indexOf(station)!==-1){ //是为了缓存已计算过的，不用重新算位置，但现在逻辑上还有误
                ...
                addAWordle(station,regions,dataS);
 ...
 */
function addAWordle(station,regions,stations) {
    console.log(station);
    let tmp=[];
        ...
        for(let dir of dirs){
            for(let leaderL=100;leaderL<=300;leaderL+=20){
                ...
                if(!nearby.length){//没有nearby 直接在origin上添加
                    addAlabel(station,regions,origin);
                    return;
                }
            }
        }
...
function wordEventHandler() {
    let wordGroup = document.querySelector('.text');
    ...
    wordGroup.addEventListener('click', function (e) {
        if (e.target && e.target.nodeName == "text") {
            d3.select(e.target).style('background-color', 'white'); //todo 没效果
        }
    })
}
//生成下拉框等
...
function getWordDict(station,query="美食",radius=2000) {
    if(station['wordDic']) return station['wordDic'];//station['wordDic']不存在才重新获取
    ...
    let locx=station.gpsx;
    let locy=station.gpsy;
    let url="http://api.map.baidu.com/place/v2/search?output=json&ak=bRPsMMG6ZZl5Gj9Haivv0NefdClWgHk0&coord-type=1&query="
    ...
    x.open('GET', url);
    // x.setRequestHeader("User-Agent":"Chrome" );
    x.send();
    ...
    //将返回数据处理成wordDic,每个word要保留uid,便于用作detail模块

    //将wordDict作为数组返回，并从大到小排列
}
